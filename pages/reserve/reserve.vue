    ...
    <block v-else>
      ...
      <view class="wrap">
        ...
        <view v-else class="slot-grid">
          <view v-for="(slot, index) in slots" :key="slot.slotId" class="slot" :class="{ on: selSlot === index && canBookSlot(slot), full: !canBookSlot(slot), paused: slot.paused }" @click="pickSlot(index)">
            <text>{{ slot.time }}</text><text>{{ slotStateText(slot) }}</text>
          </view>
        </view>
...
export default {
  data() {
    return {
      ...
      submitting: false, showRules: false, availabilityRule: '', notificationTemplateId: '', membershipExpireDate: '', shownPauseNotices: []
    }
  },
  computed: {
  ...
  methods: {
    ...
    async loadAvailability(preferredSlotId) {
      ...
      this.slots = (data.slots || []).map(slot => ({ slotId: slot.slotId, time: `${slot.startTime}-${slot.endTime}`, total: Number(slot.totalCount), booked: Number(slot.bookedCount), bookable: slot.bookable !== false, paused: slot.paused === true, pauseAnnouncement: slot.pauseAnnouncement || '' }))
      ...
      this.showPauseAnnouncement(data.pauseAnnouncements || [])
    },
    ...
    async pickSlot(index) {
      ...
      if (!this.canBookSlot(slot)) { uni.showToast({ title: slot && slot.paused ? (slot.pauseAnnouncement || '该时段暂停预约') : slot && slot.booked >= slot.total ? '该时段已约满' : '该时段已开始，不能预约', icon: 'none' }); return }
      await this.reloadAvailability(slot.slotId)
    },
    canBookSlot(slot) { return !!slot && !slot.paused && slot.bookable && slot.booked < slot.total },
    slotStateText(slot) {
      if (slot.paused) return '暂停预约'
      if (slot.booked >= slot.total) return '已约满'
      if (!slot.bookable) return '已开始'
      return `剩余 ${slot.total - slot.booked} 座`
    },
    showPauseAnnouncement(rows) {
      if (!Array.isArray(rows) || !rows.length) return
      const grouped = {}
      rows.forEach(row => {
        const announcement = String(row.announcement || '').trim()
        if (!announcement) return
        if (!grouped[announcement]) grouped[announcement] = []
        grouped[announcement].push(`${row.startTime}-${row.endTime}`)
      })
      const content = Object.keys(grouped).map(announcement => `${grouped[announcement].join('、')}\n${announcement}`).join('\n\n')
      if (!content) return
      const noticeKey = `${this.storeId}|${this.selectedDate.value}|${content}`
      if (this.shownPauseNotices.includes(noticeKey)) return
      this.shownPauseNotices.push(noticeKey)
      uni.showModal({ title: '暂停预约通知', content, showCancel: false, confirmText: '我知道了', confirmColor: '#0B756E' })
    },
    hasActiveCard(card) {
