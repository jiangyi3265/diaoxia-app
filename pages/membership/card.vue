...
export default { data(){return{card:null,loading:true,error:''}}, onShow(){this.load()}, methods:{ async load(){this.loading=true;this.error='';try{await ensureMemberSession();this.card=(await appRequest({url:'/app/me'})).card}catch(error){this.error=(error&&error.message)||'网络连接失败'}finally{this.loading=false}}, join(){uni.navigateTo({url:'/pages/membership/join'})}, code(){uni.navigateTo({url:'/pages/membership/code'})}, profile(){uni.navigateTo({url:'/pages/membership/profile'})} } }
</script>
<style scoped>
