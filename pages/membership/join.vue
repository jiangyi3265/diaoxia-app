<template>
  <view class="page">
    ...
    <view class="hero">
      ...
      <text class="hero-note">在线选座 · 动态会员码 · 到店专属权益</text>
    </view>
    ...
    <view class="content">
      ...
      <view class="rights-card">
        ...
        <view class="rights-grid">
          ...
          <view class="right-item"><view class="right-icon"><xy-icon name="card" :size="38" color="#0a968b" /></view><text>30 天权益</text></view>
      ...
      <checkbox-group class="agreement" @change="onAgreementChange">
        <label class="agreement-label">
          <checkbox class="agreement-checkbox" value="accepted" :checked="agreementAccepted" color="#0B756E" />
          <view class="agreement-copy"><text>我已阅读并同意</text><text class="agreement-link" @click.stop="openTerms">《用户服务协议》</text></view>
        </label>
      </checkbox-group>
      <button class="pay" :disabled="!selected || paying || applicationSubmitted || !agreementAccepted" :loading="paying" @click="pay">
        {{ payButtonText }}
      </button>
      </block>
    </view>
...
export default {
  data() {
    return { plans: [], selected: null, paying: false, loadingPlans: true, plansError: '', paymentMode: 'OFFLINE', applicationSubmitted: false, agreementAccepted: false }
  },
  onLoad() {
    this.load()
  ...
  methods: {
    ...
    async pay() {
      ...
      if (!this.agreementAccepted) {
        uni.showToast({ title: '请先阅读并勾选同意用户服务协议', icon: 'none' })
        return
      }
      this.paying = true
      try {
    ...
    onAgreementChange(event) {
      ...
      this.agreementAccepted = values.includes('accepted')
    }
  },
  computed: {
