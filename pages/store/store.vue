<template>
  <view class="page">
    <xy-header title="门店信息" variant="brand" :show-back="false" />
    <view class="hero">
    ...
    <view class="content">
      ...
      <view v-for="store in stores" v-if="!loading && !error" :key="store.storeId" class="store-row">
        ...
        <view v-if="store.address && !hasCoordinates(store)" class="map-tip"><xy-icon name="info" :size="25" color="#8b6b29" /><text>门店坐标待设置，当前可先复制详细地址。</text></view>
        ...
        <view class="actions">
          ...
          <button :disabled="!store.address && !hasCoordinates(store)" @click="nav(store)"><xy-icon :name="hasCoordinates(store) ? 'map-pin-nav' : 'location'" :size="28" color="#f4fbf9" /><text>{{ hasCoordinates(store) ? '地图导航' : '复制地址' }}</text></button>
        </view>
      </view>
    ...
    <view class="xy-tabbar-hold" />
    <xy-tabbar active="store" />
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    nav(store) {
      ...
      if (store.address) uni.setClipboardData({ data: store.address, success: () => uni.showToast({ title: '地址已复制', icon: 'success' }) })
    }
  }
}
</script>
