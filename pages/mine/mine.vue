<template>
  <view class="page">
    ...
    <view v-if="!loading && !error" class="section">
    ...
    <button v-if="!loading && !error" class="logout" @click="logout">退出当前账号</button>
    <view v-if="!loading && !error" class="legal-links"><text @click="go('/pages/legal/terms')">用户服务协议</text><text>·</text><text @click="go('/pages/legal/privacy')">隐私政策</text></view>
...
<script>
import { appRequest, clearMemberSession, ensureMemberSession } from '../../utils/api'
...
export default {
  ...
  methods: {
    async load() {
      ...
      catch (error) { this.error = (error && error.message) || '暂时无法读取账户信息，请检查网络后重试' }
...
.legal-links{display:flex;align-items:center;justify-content:center;gap:14rpx;margin:26rpx 0 4rpx;color:#7d908b;font-size:20rpx}.legal-links text:first-child,.legal-links text:last-child{color:#587972}
</style>
