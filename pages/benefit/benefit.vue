    ...
    <scroll-view scroll-y class="page-scroll">
      ...
      <template v-else>
        ...
        <view v-if="active && announcementConfirmed" class="seat-section">
          ...
          <view class="map-scroll-hint">座位按现场方向排列，可左右滑动查看完整座位图</view>
          <scroll-view scroll-x class="pool-map-scroll" :show-scrollbar="false">
            <view class="pool-map">
              <view class="seat-row top-row"><button v-for="n in topSeats" :key="n" :class="seatClass(n)" :aria-label="seatLabel(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ selectedSeat === n ? `✓ ${n}` : n }}</button></view>
              <view class="pool-middle">
                <view class="side-seats"><button v-for="n in [19,20]" :key="n" :class="seatClass(n)" :aria-label="seatLabel(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ selectedSeat === n ? `✓ ${n}` : n }}</button></view>
                <view class="pool"><text>33平方小池</text><text>福利钓专场</text></view>
                <view class="side-seats"><button v-for="n in [21,22]" :key="n" :class="seatClass(n)" :aria-label="seatLabel(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ selectedSeat === n ? `✓ ${n}` : n }}</button></view>
              </view>
              <view class="seat-row bottom-row"><button v-for="n in bottomSeats" :key="n" :class="seatClass(n)" :aria-label="seatLabel(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ selectedSeat === n ? `✓ ${n}` : n }}</button></view>
            </view>
          </scroll-view>
