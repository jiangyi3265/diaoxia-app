    ...
    <scroll-view scroll-y class="page-scroll">
      ...
      <template v-else>
        ...
        <view v-if="active && announcementConfirmed" class="seat-section">
          ...
          <view class="pool-map">
            <view class="seat-row top-row"><button v-for="n in topSeats" :key="n" :class="seatClass(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ n }}</button></view>
            <view class="pool-middle">
              <view class="side-seats"><button v-for="n in [19,20]" :key="n" :class="seatClass(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ n }}</button></view>
              <view class="pool"><text>33平方小池</text><text>福利钓专场</text></view>
              <view class="side-seats"><button v-for="n in [21,22]" :key="n" :class="seatClass(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ n }}</button></view>
            </view>
            <view class="seat-row bottom-row"><button v-for="n in bottomSeats" :key="n" :class="seatClass(n)" :disabled="seatUnavailable(n)" @click="selectSeat(n)">{{ n }}</button></view>
          </view>
          ...
          <view v-if="authenticated && !me.mobile" class="mobile-gate">
            <view><text>报名联系手机号</text><text>报名时需要使用微信验证手机号，普通用户无需办理会员。</text></view>
...
export default {
  data() {
    return {
      ...
      startTemplateId: '', cancelTemplateId: '', topSeats: [18,17,16,15,14,13,12,11,10], bottomSeats: [1,2,3,4,5,6,7,8,9]
    }
  },
  computed: {
    ...
    canSubmit() { return !!(this.active?.signupOpen && this.announcementConfirmed && this.selectedSeat && !this.hasActiveBooking && (!this.authenticated || this.me.mobile)) },
    submitText() {
      ...
      if (!this.me.mobile) return '请先授权手机号'
  ...
  methods: {
    ...
    async pay() {
      ...
      if (!this.me.mobile) { uni.showToast({ title: '请先授权微信手机号', icon: 'none' }); return }
...
<style scoped>
.page{height:100vh;overflow:hidden;background:#edf5f3;color:#18322e}.page-scroll{height:calc(100vh - 88rpx)}.intro{display:flex;align-items:flex-start;justify-content:space-between;margin:22rpx 24rpx 0;padding:30rpx;border-radius:34rpx;background:#7f5529;color:#fcf5e8;box-shadow:0 18rpx 38rpx rgba(91,58,24,.16)}.intro>view:first-child{display:flex;flex-direction:column}.intro-kicker{font-size:20rpx;font-weight:700;letter-spacing:3rpx;opacity:.75}.intro-title{margin-top:12rpx;font-size:40rpx;font-weight:800}.intro-note{margin-top:12rpx;font-size:22rpx;opacity:.82}.intro-mark{display:flex;align-items:center;justify-content:center;width:84rpx;height:84rpx;border-radius:26rpx;background:rgba(255,247,232,.12)}.toolbar{display:flex;align-items:center;justify-content:space-between;margin:34rpx 28rpx 16rpx}.toolbar>view,.seat-head>view:first-child{display:flex;flex-direction:column;gap:5rpx}.section-title{font-size:30rpx;font-weight:780}.section-note{color:#738984;font-size:21rpx}.toolbar button{height:62rpx;margin:0;padding:0 22rpx;border:1rpx solid #cfe2dd;border-radius:20rpx;background:#f8fcfa;color:#0b756e;font-size:22rpx;line-height:60rpx}.event-strip{width:100%;white-space:nowrap}.event-strip-inner{display:inline-flex;gap:14rpx;padding:0 24rpx 12rpx}.event-card{display:inline-flex;width:128rpx;height:154rpx;align-items:center;justify-content:center;flex-direction:column;border:2rpx solid #dce9e5;border-radius:26rpx;background:#f8fcfa}.event-card.active{border-color:#0b8d84;background:#0b8d84;color:#f4fbf9}.event-card.closed:not(.active){opacity:.7}.event-week{font-size:21rpx}.event-day{margin:8rpx 0;font-size:40rpx;font-weight:800}.event-state{font-size:18rpx;opacity:.78}.event-summary{margin:16rpx 24rpx 0;padding:26rpx;border:1rpx solid #dbe8e4;border-radius:30rpx;background:#f8fcfa}.summary-main{display:flex;justify-content:space-between;gap:16rpx}.summary-main text:first-child{font-size:29rpx;font-weight:780}.summary-main text:last-child{color:#637a74;font-size:21rpx}.summary-stats{display:grid;grid-template-columns:repeat(3,1fr);margin-top:22rpx;padding:20rpx 0;border-top:1rpx solid #e2ece9;border-bottom:1rpx solid #e2ece9}.summary-stats view{display:flex;align-items:center;flex-direction:column;gap:5rpx}.summary-stats view+view{border-left:1rpx solid #dfe9e6}.summary-stats text:first-child{font-size:28rpx;font-weight:780}.summary-stats text:last-child{color:#7b8e89;font-size:18rpx}.announcement-preview{position:relative;margin-top:20rpx;padding:18rpx 20rpx;border-radius:22rpx;background:#fbf3e4}.announcement-preview>view{display:flex;align-items:center;gap:10rpx;color:#70491d;font-size:23rpx;font-weight:750}.announcement-preview>text{display:-webkit-box;overflow:hidden;margin-top:10rpx;color:#7c654a;font-size:21rpx;line-height:1.55;-webkit-box-orient:vertical;-webkit-line-clamp:2}.announcement-preview button{height:56rpx;margin:14rpx 0 0;padding:0 19rpx;border-radius:18rpx;background:#ead8b9;color:#70491d;font-size:20rpx;line-height:56rpx}.seat-section{margin-top:28rpx;padding:0 24rpx}.seat-head{display:flex;align-items:flex-end;justify-content:space-between;gap:14rpx}.legend{display:flex;gap:11rpx;color:#7b8e89;font-size:18rpx}.legend text{display:flex;align-items:center;gap:5rpx}.legend i{display:inline-block;width:13rpx;height:13rpx;border-radius:4rpx}.legend .available{background:#f8fcfa;border:1rpx solid #bdd8d2}.legend .selected{background:#0b8d84}.legend .unavailable{background:#cfdad7}.pool-map{margin-top:18rpx;padding:18rpx 13rpx;border-radius:30rpx;background:#dfeeea}.seat-row{display:grid;grid-template-columns:repeat(9,minmax(0,1fr));gap:7rpx}.pool-middle{display:grid;grid-template-columns:56rpx minmax(0,1fr) 56rpx;gap:9rpx;margin:10rpx 0}.side-seats{display:flex;justify-content:space-around;flex-direction:column;gap:10rpx}.pool{display:flex;min-height:238rpx;align-items:center;justify-content:center;flex-direction:column;border:3rpx solid #79b7ac;border-radius:30rpx;background:#cce9e2;color:#145e55}.pool text:first-child{font-size:39rpx;font-weight:850;letter-spacing:1rpx}.pool text:last-child{margin-top:10rpx;color:#5c8179;font-size:20rpx}.map-seat{min-width:0;height:54rpx;margin:0;padding:0;border:1rpx solid transparent;border-radius:15rpx;font-size:20rpx;font-weight:750;line-height:54rpx}.map-seat.available{border-color:#c2dcd6;background:#f9fcfb;color:#385a53}.map-seat.selected{background:#0b8d84;color:#f4fbf9;box-shadow:0 7rpx 16rpx rgba(11,117,110,.22)}.map-seat.unavailable{background:#cdd9d6;color:#82918e}.my-current{display:flex;align-items:center;gap:12rpx;margin-top:16rpx;padding:18rpx 20rpx;border-radius:22rpx;background:#e3f2ed;color:#315c54;font-size:21rpx}.mobile-gate{display:flex;align-items:center;gap:15rpx;margin-top:16rpx;padding:20rpx;border:1rpx solid #e5d3b6;border-radius:24rpx;background:#fbf5e9}.mobile-gate>view{display:flex;flex:1;flex-direction:column;gap:5rpx}.mobile-gate>view text:first-child{font-size:23rpx;font-weight:750}.mobile-gate>view text:last-child{color:#7c6b55;font-size:19rpx;line-height:1.45}.mobile-gate button{height:64rpx;margin:0;padding:0 18rpx;border-radius:19rpx;background:#8a5a27;color:#fff7e9;font-size:21rpx;line-height:64rpx}.action-space{height:calc(160rpx + env(safe-area-inset-bottom))}.action-bar{position:fixed;right:0;bottom:0;left:0;z-index:40;display:flex;align-items:center;gap:20rpx;padding:18rpx 24rpx calc(18rpx + env(safe-area-inset-bottom));border-top:1rpx solid #dce8e5;background:#f8fcfa}.action-bar>view{display:flex;flex:1;flex-direction:column;gap:4rpx}.action-bar>view text:first-child{font-size:25rpx;font-weight:780}.action-bar>view text:last-child{color:#7a8e89;font-size:19rpx}.action-bar button{width:310rpx;height:82rpx;margin:0;border-radius:24rpx;background:#0b8d84;color:#f4fbf9;font-size:25rpx;font-weight:750;line-height:82rpx}.action-bar button[disabled]{background:#c3d0cd;color:#f4f8f7}.notice-overlay{position:fixed;inset:0;z-index:80;display:flex;align-items:flex-end;background:rgba(16,39,35,.46)}.notice-sheet{width:100%;padding:26rpx 26rpx calc(24rpx + env(safe-area-inset-bottom));border-radius:36rpx 36rpx 0 0;background:#f7fbfa}.notice-head{display:flex;align-items:flex-start;justify-content:space-between}.notice-head>view{display:flex;flex-direction:column;gap:7rpx}.notice-head>view text:first-child{font-size:32rpx;font-weight:800}.notice-head>view text:last-child{color:#718681;font-size:21rpx}.notice-head button{display:flex;width:64rpx;height:64rpx;align-items:center;justify-content:center;margin:0;padding:0;border-radius:20rpx;background:#e8efed}.notice-scroll{max-height:660rpx;margin-top:20rpx;padding:22rpx;box-sizing:border-box;border-radius:24rpx;background:#edf4f2}.notice-scroll>text{white-space:pre-wrap;color:#334e48;font-size:23rpx;line-height:1.75}.notice-rule{display:flex;justify-content:space-between;gap:18rpx;padding:19rpx 0;border-bottom:1rpx solid #dbe7e4;font-size:21rpx}.notice-rule:first-of-type{margin-top:20rpx;border-top:1rpx solid #dbe7e4}.notice-rule text:first-child{color:#718681}.notice-rule text:last-child{color:#29463f;text-align:right}.notice-confirm{height:88rpx;margin-top:20rpx;border-radius:25rpx;background:#0b8d84;color:#f4fbf9;font-size:27rpx;font-weight:780;line-height:88rpx}.empty{display:flex;align-items:center;flex-direction:column;padding:110rpx 36rpx;text-align:center}.empty-icon{display:flex;width:106rpx;height:106rpx;align-items:center;justify-content:center;border-radius:33rpx;background:#dfece8}.empty-title{margin-top:24rpx;font-size:29rpx;font-weight:760}.empty-note{max-width:28em;margin-top:11rpx;color:#748984;font-size:21rpx;line-height:1.6}
</style>
